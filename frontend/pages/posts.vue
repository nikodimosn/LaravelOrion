        ...
        <div class="bg-white rounded shadow p-6 m-4 w-full lg:w-3/4 lg:max-w-lg">
            ...
            <div>
                <div class="flex mb-4 items-center" v-for="(todo,i) in todos" :key="todo.id">
                    ...
                    <p :class="[todo.is_done ? 'line-through text-green-600' : '', `w-full text-grey-darkest font-semibold text-gray-600`]">{{ todo.title }}</p>
                    ...
                    <button v-else class="flex-no-shrink p-2 ml-4 mr-2 border-2 rounded-lg border-green text-white bg-green-500 hover:bg-green-700" @click="updateStatus(todo,i)">Show</button>
...
export default {
  ...
  async fetch(){
    ...
    this.todos = await fetch(`http://localhost:8000/api/postx`).then(res=>res.json())
  }
}
</script>
